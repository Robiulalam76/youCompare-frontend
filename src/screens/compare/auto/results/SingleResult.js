...
const Text = (props) => <Typography {...props} variant="body1"></Typography>;
...
const Quote = ({ autoquote, toggleDetailsShow, showDetails }) => {
  ...
  const { netPremium } = premiumCalculator(
    premiums,
    discounts,
    vatInParcentage
  );
  ...
  return (
    <Grid container spacing={1}>
      ...
      <StyledGrid item xs={12} sm={6} md={3}>
        <ButtonBase
          disableRipple
          sx={{ mr: 1, py: 0.25, color: "primary.main" }}
...
const ItemDisplay = ({ title }) => {
  return (
    <Box sx={{ display: "flex", alignItems: "center", pt: 0.5 }}>
      <CircleOutlinedIcon
        sx={{ fontSize: 12, mr: 0.5, color: "primary.main" }}
      />
      <Typography
        variant="body2"
        sx={{ fontWeight: "normal", lineHeight: "27px" }}
      >
        {title}
      </Typography>
    </Box>
...
const PremuimBreakup = ({ premiums, discounts }) => {
  ...
  const { packagePremium, vat, netPremium } = premiumCalculator(
    premiums,
    discounts,
    vatInParcentage
  );
      ...
      <Grid container>
        <Grid item xs={12} sm={8} md={6}>
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <FiberManualRecordIcon sx={{ fontSize: 10, m: 1 }} />
...
const PremiumDisplay = ({ title, value }) => {
  ...
  return (
    <Box sx={{ display: "flex", justifyContent: "space-between", width: 1 }}>
      <Typography variant="body2" gutterBottom style={fontStyle}>
        {title}
      </Typography>
      <Typography variant="body2" gutterBottom style={fontStyle}>
        &#8358; {value}
