                                    ...
                                    <div className='flex items-center flex-wrap gap-2 md:gap-4 mt-2 w-full'>
                                        {
                                            item?.children?.map(item => (
                                                <button onClick={() => setSelectedTab(item?.id)}
                                                    key={item.id}
                                                    className={`flex justify-center items-center h-8 rounded-3xl w-fit px-2 text-sm 
                                                    ${selectedTab === item?.id ? 'bg-sky-600 text-white' : 'bg-white border border-sky-600 text-sky-600'}`}
                                                    value={item?.title}>
